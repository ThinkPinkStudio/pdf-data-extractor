#!/usr/bin/env node
// PROBE TEMPORANEA — A/B di CONFERMA del fix recency/evidenze-OCR (motore a stadi).
// FEDELE alla pipeline di produzione: OCR spaziale in cache out/ocr/ (già pronta,
// NON si rigenera), profilo RC PROF MED V2, staged gruppi (perField=false, cascade=false).
...
const OUT = 'out/probe_confirm_recency.json'
...
// ── fascicolo dalla cache OCR (pagine SPAZIALI già pronte) ──────────────────────
...
for (const f of pdfs) {
  ...
  if (!Array.isArray(pages) || !pages.length) {
    console.log(`  ⚠ cache OCR mancante per ${f} — salto`)
    continue
  }
  ...
  console.log(`— ${f} → ${spatial.length} pagine da cache, ${chars} char`)
}
if (!docs.length) { console.error('Nessun documento utilizzabile in cache OCR'); process.exit(2) }

const t0 = Date.now()
console.log(`\nLancio estrazione staged-gruppi su ${OLLAMA_URL} / ${MODEL} … (${docs.length} file, ${fields.length} campi, numCtx 24576)\n`)
let result
try {
...
const out = {
  profilo: profilo.name, fascicolo: FASCICOLO, modello: MODEL, secs,
