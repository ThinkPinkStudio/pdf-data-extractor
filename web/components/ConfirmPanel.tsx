 ...
 */
// hideCancel: solo il pulsante OK (avviso, non domanda: es. azione rifiutata dal server).
export interface ConfirmOptions { okLabel?: string; cancelLabel?: string; danger?: boolean; title?: string; details?: ReactNode; hideCancel?: boolean }
  ...
  const panel = state ? (
    <div role="dialog" aria-modal="false" style={{
      ...
      <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
        {!state.opts.hideCancel && <button type="button" className="btn btn-secondary" style={{ fontSize: 12 }} onClick={() => close(false)}>{state.opts.cancelLabel || 'Annulla'}</button>}
