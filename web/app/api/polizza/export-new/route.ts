...
export async function POST(req: NextRequest) {
  ...
  const { data, suggestedName, fields } = await req.json().catch(() => ({}))
  if (!data || typeof data !== 'object') {
    return NextResponse.json({ error: 'Nessun dato da esportare' }, { status: 400 })
  }
  try {
    // campi del job (se inviati): l'export segue il profilo con cui è stato
    // estratto il dossier, non quello attivo nelle Impostazioni
    const buf = await exportNewExcel(data, Array.isArray(fields) ? fields : null)
