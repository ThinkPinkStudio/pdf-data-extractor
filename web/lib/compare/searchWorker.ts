...
import { compare, runEqualByKeys, runInclusionSearch, runBothByRow, type Row, type Condition, type MatchKey, type FuzzyOpts } from './engine'
...
ctx.onmessage = (e: MessageEvent<Req>) => {
  ...
  try {
    ...
    } else if (msg.kind === 'equal') {
      // «Uguale a»: righe di A presenti in B per chiavi + fuzzy sui non trovati
      ctx.postMessage({ ok: true, result: runEqualByKeys(msg.dataA, msg.dataB, msg.keys, msg.fuzzy) })
    } else {
